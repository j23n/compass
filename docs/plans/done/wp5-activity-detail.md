...
1. ~~**Task 0** — Rename `totalCalories` → `activeCalories`~~ ✅ done (651a462)
...
## Task 0 — Rename `totalCalories` → `activeCalories` ✅ DONE

Shipped in commit 651a462. The model field, parser, UI label ("Active Cal"), and call sites have all been renamed.
...
Sport is a `String`-rawValue enum. Add new cases:
...
**FIT sport codes — note on the move to FitFileParser.** The parser no longer reads numeric field offsets; `ActivityFITParser` (post-rewrite) calls `FitMessage.interpretedField(key: "sport")?.name` and `interpretedField(key: "sub_sport")?.name`, which return FIT enum *string names* (e.g. `"cycling"`, `"mountain"`, `"strength_training"`). So:

- The `fitSportCode: UInt8` property on `Sport` is **only** used by `CourseFITEncoder` (course writing), not by parsing. Keep it for encoding and add the new cases there:
...
public var fitSportCode: UInt8 {
    ...
    case .strength:     return 10  // training
    case .cardio:       return 0   // generic
    case .other:        return 0
    }
}
```

(No `fitSubSportCode` property needed — `CourseFITEncoder` does not currently emit sub_sport. If it ever does, MTB is sub_sport `8` under sport `2`.)

**MTB and other new sports — parser mapping (`ActivityFITParser.mapSport`).** Extend the existing string-based switch. FitFileParser exposes the relevant strings as:

| Sport case | sport string | sub_sport string |
|---|---|---|
| `.mtb` | `"cycling"` | `"mountain"` (also consider `"downhill"`) |
| `.rowing` | `"rowing"` | — |
| `.kayaking` | `"kayaking"` | — |
| `.skiing` | (none — Garmin uses sub_sport under cycling/training; verify with a real ski FIT) | — |
| `.snowboarding` | `"snowboarding"` | — |
| `.sup` | `"stand_up_paddleboarding"` | — |
| `.climbing` | — | `"mountaineering"` (sub_sport 16) |
| `.boating` | `"boating"` | — |

Update `mapSport(from:)` to:
...
private func mapSport(from session: FitMessage) -> Sport {
    ...
    // sub_sport refinements first
    switch subSport {
    case "yoga", "pilates":     return .yoga
    case "strength_training":   return .strength
    case "mountain", "downhill" where sport == "cycling": return .mtb
...
(Skiing string mapping: confirm with a real alpine-ski FIT before adding — the Garmin SDK exposes several variants and the canonical `sport`-level string is not obvious from the rzfit map.)
...
If the values are zero (parser not reading them for cycling), the issue is in `ActivityFITParser.swift`: `buildActivity` reads `doubleValue(session, key: "total_ascent" / "total_descent")` from the **session** message only — currently no `lap` aggregation. If Garmin is writing ascent/descent at the lap level for cycling but not session, extend the parser to sum lap ascents/descents (the `case .lap` arm currently only logs).
...
| `Packages/CompassFIT/Sources/CompassFIT/Parsers/ActivityFITParser.swift` | 1 (extend `mapSport` with new sport/sub_sport string cases), 7 (lap-level ascent/descent fallback if needed) |
